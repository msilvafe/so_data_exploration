...
def get_dict_entry(base_dir, entry, config):
    ...
    try:
        ...
        # Q
        m = has_all_cut(x.noiseQ_fit.valid)
        keys.append('Qfknee')
        vals.append(x.noiseQ_fit.fit[m,0])
        keys.append('Qwnl')
        vals.append(x.noiseQ_fit.fit[m,1])
        keys.append('Qalpha')
        vals.append(x.noiseQ_fit.fit[m,2])
        m = has_all_cut(x.noiseQ_nofit.valid)
        keys.append('Qwnl_nofit')
        vals.append(x.noiseQ_nofit.white_noise[m])
        # U
        m = has_all_cut(x.noiseU_fit.valid)
        keys.append('Ufknee')
        vals.append(x.noiseU_fit.fit[m,0])
        keys.append('Uwnl')
        vals.append(x.noiseU_fit.fit[m,1])
        keys.append('Ualpha')
        vals.append(x.noiseU_fit.fit[m,2])
...
def main(arxiv, configs, nproc, errlog_ext, savename):
    ...
    np.save(os.path.join('/global/homes/m/msilvafe/so_home/shared_files/satp2_noise_check', savename), outdict)
