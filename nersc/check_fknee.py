...
def get_dict_entry(base_dir, entry, config):
    ...
    try:
        ...
        configs,ctx = pp_util.get_preprocess_context(config)
        ...
        for fld in test._fields:
            outdict['fit_noiseT'] = test.noiseT_fit.fit
            outdict['nofit_noiseT'] = test.noiseT_nofit.white_noise
            outdict['fit_noiseQ'] = test.noiseQ_fit.fit
            outdict['nofit_noiseQ'] = test.noiseQ_nofit.white_noise
            outdict['fit_noiseU'] = test.noiseU_fit.fit
            outdict['nofit_noiseU'] = test.noiseU_nofit.white_noise
    ...
    with ProcessPoolExecutor(nproc) as exe:
        ...
        for future in as_completed(futures):
            ...
            if did is None:
                ...
                np.save(os.path.join('/global/homes/m/msilvafe/so_home/shared_files/fknee_check', savename), outdict)
            i += 1
