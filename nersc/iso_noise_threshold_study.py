...
def get_dict_entry(base_dir, entry, config):
    try:
        ...
        m = has_all_cut(x.noise_signal_nofit.valid)
        keys.append('noise_nofit_white')
        #vals.append(x.white_noise_nofit.white_noise[m])
        vals.append(x.noise_signal_nofit.white_noise[m])
        t4 = time.time()
        logger.info(f'{t4-t3} sec to run white noise {entry["dataset"]}')
        # low f noise
        #m = has_all_cut(x.Pxx_raw.valid)
        #mf = (x.Pxx_raw.freqs > 0.001) & (x.Pxx_raw.freqs < 0.01)
        #pn = x.Pxx_raw.Pxx[m]
        #keys.append('noise_lowf')
        #vals.append(np.median(pn[:, mf], axis=1))
        #t5 = time.time()
        #logger.info(f'{t5-t4} sec to run low f noise {entry["dataset"]}')
        #logger.info(f'{t5-t0} sec to run everything {entry["dataset"]}')

        # Total yield
        ### Changed this to split_flags
